...
class LearningAnalytics {
    ...
    checkAndAnalyze() {
        ...
        const daysSinceLastAnalysis = Math.floor((now - lastAnalysis) / (1000 * 60 * 60 * 24));
        
        // 每3天分析一次，或者积累了足够多的数据（30题）
        const recentSessions = this.getRecentSessions(3);
        const recentProblems = recentSessions.reduce((sum, s) => sum + s.correct + s.wrong, 0);
        
        if (daysSinceLastAnalysis >= 3 || recentProblems >= 30) {
            this.performAnalysis();
        }
    }
    ...
    performAnalysis() {
        ...
        if (recentAccuracy >= 0.85 && this.data.currentLevel < 8) {
            ...
            reason = `最近3天正确率达到 ${(recentAccuracy * 100).toFixed(1)}%，表现优秀，建议提升难度！`;
        } else if (recentAccuracy < 0.5 && this.data.currentLevel > 1) {
            recommendation = 'downgrade';
            reason = `最近3天正确率为 ${(recentAccuracy * 100).toFixed(1)}%，建议降低难度巩固基础。`;
        } else {
            reason = `最近3天正确率为 ${(recentAccuracy * 100).toFixed(1)}%，继续保持当前难度练习。`;
        }
        ...
        modal.innerHTML = `
            <div style="background: white; border-radius: 24px; padding: 25px; max-width: 420px; width: 100%; max-height: 85vh; overflow-y: auto; animation: slideUp 0.3s ease;">
                <div style="text-align: center; margin-bottom: 20px;">
                    ...
                    <p style="color: #888; margin: 5px 0;">最近3天表现分析</p>
                </div>
